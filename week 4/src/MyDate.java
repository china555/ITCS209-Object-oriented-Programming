...
public class MyDate {
	...
	public MyDate nextMonth() {
		if (this.month == 12) {
			...
			if (this.month == 3 || this.month == 5 || this.month == 8 || this.month == 7|| this.month == 10) {
				if (this.day == 31) {
					this.month++;
					this.day = 30;
				} else
	...
	public int yeardiff2() {
		
		return 0;
	}
	public int yearDiff(MyDate start,MyDate end) {
		if(end.year < start.year) {
			return -1;
		}
		else {
			LocalDate myBirthDate = LocalDate.of(start.year, start.month,start.day);
			LocalDate now = LocalDate.of(end.year, end.month, end.day);
			
			Period diff = Period.between(myBirthDate, now);
