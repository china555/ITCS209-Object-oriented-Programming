...
public class Player {
	...
	public void takeAction(Arena arena) {
		...
		Player[][] enemy = null;
		if(arena.alliance(this) == arena.getTeamA()) {
			 alliance = arena.getTeamA();
			 enemy = arena.getTeamB();	
		}
		else if(arena.alliance(this) == arena.getTeamB()) {
			 alliance = arena.getTeamB();
			 enemy = arena.getTeamA();
		}
		if(nubturn == this.ultiturn) {
		switch(this.type) {
		case Healer:
			useSpecialAbility(alliance, enemy);
			break;
		case Tank:
			useSpecialAbility(alliance, enemy);
			break;
		...
		for (int i = 0; i < 2; i++) {
			for (int j = 0; j < enemy[i].length; j++) {
				if(enemy[i][j].isAlive() ==false) {
					enemy[i][j].isinfected = false;
					enemy[i][j].isProtecting = false;
					enemy[i][j].isPassout = false;
				}
			}
		}
}
